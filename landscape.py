...
class Landscape():
    ...
    def _initialise_patches(self):
        ...
        while self.sheepCount < INIT_NUMBER_SHEEP:
            ...
            if self.patches[x_coord][y_coord].sheep==None:
                self.sheep[self.sheepIDTracker] = Sheep(x_coord, y_coord, self.sheepIDTracker)
                #self.sheep.append(Sheep(x_coord, y_coord))
                self.patches[x_coord][y_coord].sheep = self.sheep[self.sheepCount]
                self.sheepCount += 1
                self.sheepIDTracker += 1 # REFACTOR: LET SHEEP CLASS DEAL WITH THIS
            x_coord = rnd.randint(0,COLUMNS-1)
            y_coord = rnd.randint(0,ROWS-1)        
        # create wolves    
        x_coord = rnd.randint(0,COLUMNS-1)
        ...
        while self.wolfCount < INIT_NUMBER_WOLVES:
            ...
            if self.patches[x_coord][y_coord].wolf==None:
                self.wolves[self.wolfIDTracker] = Wolf(x_coord, y_coord, self.wolfIDTracker)                
    ...
    def update(self):
        ...
        for w in self.wolves:
            ...
            if (eat_ID!=None):
                eatenSheep = self.sheep[eat_ID]
                self.patches[eatenSheep.i][eatenSheep.j].sheep = None
                del self.sheep[eat_ID]
                self.sheepCount -= 1
                print("Sheep eaten")
